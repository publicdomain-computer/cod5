    ...
    char *argv[])
{
    ...
    printf("  Image type: %d\n", getbyte());
	printf("  Version pos: %d\n", getbyte());

    hdrcnt = getbyte();					// Get count for header
    printf("  Header size: %d\n", hdrcnt);
    if (filever != 2)
    {
        puts("? RUNDMP: File version not 2, cannot continue");
        exit(1);
    }
    if (hdrcnt < 56)
    {
        fprintf(stderr, "? RUNDMP: Header byte count (%d) less than 56, cannot "
                "continue\n", hdrcnt);
        exit(1);
    }
	...
	printf(  "  Offset of exported symbol table: 0x%08.8X\n"
			...
			"  Size of imported symbol table: %d\n", exportoffset, exportnum,
			importoffset, importnum);
	fileid1 = getlong();
	fileid2 = getlong();
	fileid3 = getlong();
    ...
    while (--segcnt >= 0)
    {
        printf("\n**** Segment header (segment #%d) at %08X\n", ++segnum,
				ftell(fp));
        hdrcnt = getbyte();				// Get count for header
        printf("  Byte count: %d\n", hdrcnt);
        if (hdrcnt < 12)
    ...
    while (--msectcnt >= 0)
    {
        if (msectpnt->ml_relocsz)
        {
            printf("\n**** Relocation records for msect #%d (%ld record%s) "
                    ...
                    (msectpnt->ml_relocsz == 1)? "":"s", msectpnt->ml_relocos);
